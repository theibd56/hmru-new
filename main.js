...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initSliders() {
        ...
        const sliders = document.querySelectorAll('.promo-catalog__slider');

        if (screenWidth < 992) {
            sliders.forEach((slider, index) => {
                if (!catalogSliders[index]) {
                    catalogSliders[index] = new Swiper(slider, {
                        slidesPerView: 2.2,
                        spaceBetween: 10,
                        speed: 600,
                        loop: false,
                        pagination: {
                            el: slider.querySelector('.products-pagination'),
                            clickable: true,
                        },
                        autoplay: {
                            delay: 6000,
            ...
            sliders.forEach((slider, index) => {
                if (catalogSliders[index]) {
                    catalogSliders[index].destroy(true, true);
                    catalogSliders[index] = null;
                }
            });
        }
    }
